...
要求 DeepSeek Harness `0.1.2-rc.1` 至 `0.1.7-alpha.2`（prerelease 系列在 `peerDependencies` 里逐个钉住），且宿主 PATH 里有 git。工作区不在 git 仓库内时，标签页提供文件浏览与一键初始化，而不是审查工作台——绝不白屏。显示偏好的存放位置随宿主走：`0.1.7-alpha.1` 起是宿主为本插件条目服务的那份配置（本插件用 `.volatile()` 字段声明、按 profile 条目 id 读取），`0.1.6-alpha.2` 起在侧栏 Plugins 页里本插件的条目，更早则是设置弹窗的插件卡片。
