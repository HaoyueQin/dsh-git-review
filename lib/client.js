window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function badgesFor(file) {
			...
			if (/[UA]{2}|U[AD]|DU/.test(file.x + file.y)) return [{
				glyph: "U",
				key: "conflict",
				tone: "Error"
			}];
		...
		function ReviewView({ cwd, settings, t, useSession, useInput, inputActions }) {
			...
			const conflictCount = (0, react.useMemo)(() => ready === null ? 0 : ready.files.filter((file) => /[UA]{2}|U[AD]|DU/.test(file.x + file.y)).length, [ready]);
