	...
	factory: (require) => {
		...
		function ReviewView({ cwd: injectedCwd, sessionId, sessionsList, settings, t, useSession, useInput, inputActions }) {
			...
			const startGraphResize = (0, react.useCallback)((event) => {
				...
				const clamp = (value) => Math.min(900, Math.max(180, value));
				let raf = 0;
				let latest = 0;
				const onMove = (move) => {
				...
				const onUp = (up) => {
					...
					if (state === null) return;
					try {
						localStorage.setItem("dsh-git-review.graphWidth", String(Math.round(clamp(state.startWidth + (up.clientX - state.startX)))));
					} catch {}
				};
				dragCleanupRef.current?.();
				dragCleanupRef.current = done;
