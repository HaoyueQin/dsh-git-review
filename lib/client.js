window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function TreePanel({ files, selected, onSelect, filter, onFilterChange, collapsed, onToggleDir, mode, onModeChange, listFailed, matchCounts, t }) {
		...
		async function loadStatus(cwd, base) {
		...
		async function loadFileDiff(cwd, path, origPath, untracked, full, scope, base) {
		...
		function ReviewView({ cwd, t, useSession, useInput, inputActions }) {
			...
			(0, react.useEffect)(() => {
				...
				loadStatus(cwd, baseRef).then((next) => {
			...
			(0, react.useEffect)(() => {
				if (treeMode !== "all" || cwd === void 0) return;
				let alive = true;
			...
			const allRows = (0, react.useMemo)(() => treeMode === "all" && allFiles !== null ? mergeAllFiles(allFiles, ready?.files ?? []) : null, [
				treeMode,
				allFiles,
				ready
			]);
			...
			(0, react.useEffect)(() => {
				...
				(selectedFile.unchanged === true || effectiveView === "file" ? loadFileContent(cwd, selected) : loadFileDiff(cwd, selected, selectedFile.origPath, selectedFile.untracked, diffFull, diffScope, baseRef)).then((next) => {
							...
							(0, react_jsx_runtime.jsxs)("label", {
								...
								children: [
									...
									(0, react_jsx_runtime.jsxs)("select", {
										...
										}), (refs ?? []).map((ref) => (0, react_jsx_runtime.jsx)("option", {
											value: ref.name,
											children: ref.name
										}, ref.kind + ":" + ref.name))]
									}),
