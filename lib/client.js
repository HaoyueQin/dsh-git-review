window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function highlightedLine(text, engine, tokens) {
			...
			const render = (chunk) => {
				...
				return tokens.map((token, index) => {
					...
					const cls = token.kind === "kw" ? review_module_css_default.tokKw : token.kind === "str" ? review_module_css_default.tokStr : token.kind === "num" ? review_module_css_default.tokNum : review_module_css_default.tokCom;
					return (0, react_jsx_runtime.jsx)("span", {
						className: cls,
						children: inner
					}, index);
			...
			return parts.map((part, index) => typeof part === "string" && index % 2 === 1 ? (0, react_jsx_runtime.jsx)("mark", {
				className: review_module_css_default.matchMark,
				children: part
			}, index) : typeof part === "string" ? render(part) : part);
		}
		...
		function FilePane({ file, search, content, truncated, binary, size, loading, canShowDiff, view, onViewChange, syntaxHighlight, t }) {
			const engine = (0, react.useMemo)(() => makeSearchEngine(search), [search]);
			const highlighter = (0, react.useMemo)(() => syntaxHighlight ? makeLineHighlighter(file.path) : null, [file.path, syntaxHighlight]);
		...
		function renderTokens(text, engine, tokens) {
			...
			return tokens.map((token, index) => {
				...
				return token.kind === "kw" || token.kind === "str" || token.kind === "num" || token.kind === "com" ? (0, react_jsx_runtime.jsx)("span", {
		...
		function DiffPane({ file, diff, truncated, loading, binary, size, full, onToggleFull, scope, onScopeChange, view, onViewChange, showViewSwitch = true, allowFileView = true, wsIgnore, onToggleWs, syntaxHighlight, search, baseActive, useInput, inputActions, onDraftAdd, t }) {
		...
		function prefsEqual(a, b) {
			return a.viewMode === b.viewMode && a.searchScope === b.searchScope && a.graphCollapsed === b.graphCollapsed && a.searchCS === b.searchCS && a.searchRegex === b.searchRegex && a.wsIgnore === b.wsIgnore && a.syntaxHighlight === b.syntaxHighlight;
		}
