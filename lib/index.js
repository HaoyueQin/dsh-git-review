...
async function gitBlame(cwd, path) {
	...
	const repoRoot = await resolveRepository(cwd);
	if (repoRoot === null) throw new Error("not a git repository");
	const rows = parseBlamePorcelain(await runGit(repoRoot, [
		"blame",
		"--porcelain",
		"--",
	...
	return {
		...
		truncated: rows.length > BLAME_LINE_CAP
	};
}
...
//#endregion
export { apply, gitBlame, gitCherryPick, gitCommit, gitConflictFinish, gitConflictResolve, gitDiscard, gitEnv, gitFetch, gitFileContent, gitFileDiff, gitFileHistory, gitHunkOp, gitLastCommit, gitMerge, gitPull, gitReset, gitRevert, gitStage, gitStash, gitStatus, gitUnstage, inject, name };
