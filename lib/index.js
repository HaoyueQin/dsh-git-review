...
async function resolveBaseCommit(repoRoot, ref) {
	try {
...
async function gitStatus(cwd, base) {
	...
	const overrideCommit = baseRef === null ? null : await resolveBaseCommit(repoRoot, baseRef).then((commit) => commit !== null && commit !== headCommit ? commit : null);
...
async function gitFileDiff(cwd, path, untracked, full, origPath, scope, base) {
	if (typeof cwd !== "string" || cwd === "") throw new Error("cwd is required");
	const repoRoot = await resolveRepository(cwd);
