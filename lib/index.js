...
function sniffPreviewMime(bytes) {
	...
	if (bytes.length >= 6 && ascii(0, "GIF87a") || bytes.length >= 6 && ascii(0, "GIF89a")) return "image/gif";
	...
	if (ascii(at, "<svg")) return "image/svg+xml";
	return null;
}
//#endregion
...
async function readPreviewBytes(cwd, path, ref) {
	if (typeof cwd !== "string" || cwd === "") throw new Error("cwd is required");
	const repoRoot = await resolveRepository(cwd);
	if (repoRoot === null) throw new Error("not a git repository");
	const absPath = fenceRepoPath(repoRoot, typeof path === "string" ? path : "");
...
async function gitFileBytes(cwd, path, ref) {
...
async function gitBlame(cwd, path) {
	if (typeof cwd !== "string" || cwd === "") throw new Error("cwd is required");
	const repoRoot = await resolveRepository(cwd);
	if (repoRoot === null) throw new Error("not a git repository");
	const streamed = await runGitStreamed(repoRoot, [
		...
		"--porcelain",
		"--",
		relative(repoRoot, fenceRepoPath(repoRoot, typeof path === "string" ? path : "")).replaceAll("\\", "/")
	]);
...
async function gitFileHistory(cwd, path) {
	...
	return {
		ok: true,
		commits,
		truncated: commits.length >= FILE_HISTORY_CAP
	};
}
...
async function gitSearch(cwd, query, base, target, mode, cs, rx) {
	...
	if (mode === "content") {
		...
		for (const relPath of untracked.slice(0, SEARCH_UNTRACKED_CAP)) {
			...
			try {
				if (!(await lstat(absPath)).isFile()) continue;
...
async function gitOpenWith(cwd, path, app, confirm) {
	...
	const run = () => new Promise((resolvePromise, rejectPromise) => {
		const argv = tool !== void 0 ? [tool, tool === "explorer.exe" ? "/select," + absPath : absPath] : process.platform === "win32" ? ["explorer.exe", absPath] : ["xdg-open", absPath];
...
//#endregion
export { apply, gitBlame, gitBranchCreate, gitBranchDelete, gitBranchRename, gitBranchSwitch, gitBranchTrack, gitCherryPick, gitCommit, gitCommitFiles, gitConflictFinish, gitConflictResolve, gitDiscard, gitEnv, gitFetch, gitFileBytes, gitFileContent, gitFileDiff, gitFileHistory, gitFileOp, gitHunkOp, gitLastCommit, gitListFiles, gitLog, gitMerge, gitPull, gitRefs, gitReset, gitRevert, gitSearch, gitStage, gitStash, gitStatus, gitTagCreate, gitTagDelete, gitTagPush, gitUnstage, inject, name };
