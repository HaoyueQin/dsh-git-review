...
function live(field) {
	const marker = field.volatile;
	return typeof marker === "function" ? marker.call(field) : field;
}
const ReviewSettingsSchema = z.object({
	viewMode: live(z.union(["split", "unified"]).default("split").loose()),
	searchScope: live(z.union([
		"diff",
		"content",
		"path"
	]).default("diff").loose()),
...
function runGitBytes(root, args, byteCap = PREVIEW_CAP, timeoutMs = GIT_TIMEOUT_MS) {
	return new Promise((resolvePromise) => {
		...
		child.once("close", (code) => {
			...
			clearTimeout(timer);
			resolvePromise({
				code: truncated && !timedOut ? 0 : timedOut ? -1 : code ?? -1,
				data: Buffer.concat(out),
				stderr: Buffer.concat(err).toString("utf8"),
				truncated
			});
...
async function gitRefs(cwd) {
	...
	refs.sort((a, b) => order[a.kind] !== order[b.kind] ? order[a.kind] - order[b.kind] : compareNatural(a.name, b.name));
	return {
		ok: true,
...
async function gitLog(cwd, limit, skip) {
	...
	const skipCount = typeof skip === "number" && Number.isFinite(skip) && skip >= 1 ? Math.min(Math.floor(skip), 1e5) : 0;
	{
		...
		raw = streamed.truncated ? streamed.stdout.slice(0, streamed.stdout.lastIndexOf("") + 1) : streamed.stdout;
...
async function gitSearch(cwd, query, base, target, mode, cs, rx, ws = false) {
	...
	if (mode === "content") {
		...
		if (targetRef !== null) {
			...
			return {
				ok: true,
				matches: parseGrepCounts(result.stdout, targetCommit).sort((a, b) => b.count - a.count || compareNatural(a.path, b.path)),
				truncated: false
			};
		}
