 ...
 *
 * Contract source: deepseek-harness-background docs/GLASS_API.zh.md (v1).
 * Mode decision: 'fill' — the review root paints with
 * `--dsw-alias-bg-layer-1` (outside the six-token auto-transparency table),
 * so the registry must take over the fill as well as the blur chain. When
 * the glass is off every surface falls back to its own stylesheet paint.
 */
